package com.example.demo.alert.infrastructure.messaging.mapper;

import com.example.demo.alert.domain.signal.IndicatorSignal;
import com.example.demo.contracts.message.trade_indicator.PremiumIndicatorMessage;
import com.example.demo.infra_shard.messaging.mapper.MessageToDomain;
import lombok.RequiredArgsConstructor;
import org.springframework.stereotype.Component;

@Component
@RequiredArgsConstructor
public class PremiumIndicatorMessageToSignal implements MessageToDomain<PremiumIndicatorMessage, IndicatorSignal> {
    @Override
    public IndicatorSignal toDomain(PremiumIndicatorMessage message) {
        return new IndicatorSignal(
                message.symbol(),
                SignalValueSupport.parseDecimalOrZero(message.value()),
                SignalValueSupport.parseLongOrZero(message.observeCloseTs()),
                "premium",
                message.interval(),
                message.type(),
                message.period(),
                SignalValueSupport.parseLongOrZero(message.baseExchangeId()),
                SignalValueSupport.parseLongOrZero(message.compareExchangeId())
        );
    }
}
