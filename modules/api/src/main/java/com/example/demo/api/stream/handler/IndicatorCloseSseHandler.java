...
public class IndicatorCloseSseHandler {
    ...
    public SseEmitter subscribe(String type) {
        ...
        Disposable subscription = switch (type) {
            case "premium" -> analyticsStream.premiumIndicatorSink.asFlux()
                    .subscribe(
                            msg -> sendEvent(emitter, "indicator-close", msg),
                            err -> emitter.completeWithError(err),
                            emitter::complete
                    );
            default -> analyticsStream.tickIndicatorSink.asFlux()
                    .subscribe(
                            msg -> sendEvent(emitter, "indicator-close", msg),
                            err -> emitter.completeWithError(err),
                            emitter::complete
