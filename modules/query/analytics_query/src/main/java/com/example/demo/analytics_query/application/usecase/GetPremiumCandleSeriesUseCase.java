...
public class GetPremiumCandleSeriesUseCase {
    ...
    public CursorSlice<PremiumCandleView> executeCursor(String symbol, Long baseExchangeId, Long compareExchangeId,
                                                        String interval, Long cursor, int limit,
                                                        CursorDirection direction) {
        if (limit <= 0) {
            throw new IllegalArgumentException("limit must be positive");
        }
        List<PremiumCandleView> fetched = port.findCursor(
                symbol, baseExchangeId, compareExchangeId, interval, cursor, limit + 1, direction);
