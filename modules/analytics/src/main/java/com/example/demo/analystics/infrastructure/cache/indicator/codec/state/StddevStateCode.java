...
public class StddevStateCode implements IndicatorStateCodec<StddevState> {
    private static final Charset UTF8 = StandardCharsets.UTF_8;
    private static final char SEP = '|';

    @Override
    public byte[] encode(StddevState state,long timestamp) {
    ...
    public RecoveryIndicatorState decode(byte[] bytes) {
        ...
        BigDecimal mean = new BigDecimal(str.substring(p1 + 1, p2));
        BigDecimal m2 = new BigDecimal(str.substring(p2 + 1, p3));
        long timestamp = Long.parseLong(str.substring(p3 + 1));

        StddevState state = new StddevState(count,mean,m2);
        return new RecoveryIndicatorState(state, timestamp);
    }

    @Override
    public Set<TradeIndicatorType> supportedTradeIndicatorTypes() {
        return Set.of(TradeIndicatorType.STDDEV);
    }
}
